...
public class TendenciaCompraPDF {

    public void generalHistoricoTendenciaCompraPDF(
            String fechaInicio,
            String fechaFin,
            List<Object[]> historialVenta) {
        try {
            // Ruta de descargas
            String ruta = System.getProperty("user.home") + "/Downloads/HistoricoTendenciaCompra_" + fechaInicio + "_a_"
                    + fechaFin
            ...
            Paragraph titulo = new Paragraph("Histórico de tendencia de compra", tituloFont);
            titulo.setAlignment(Element.ALIGN_CENTER);
            doc.add(titulo);

            // Agregar la fechas (Inicio - Fin)
            Font subFont = new Font(Font.HELVETICA, 12, Font.NORMAL, Color.GRAY);
            doc.add(new Paragraph("Periodo: " + fechaInicio + " del " + fechaFin, subFont));
            ...
            doc.close();
            System.out.println(" Historico generado en: " + ruta);
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println(" Error al generar el PDF: " + e.getMessage());
            JOptionPane.showMessageDialog(null, "Hubo un problema al generar el PDF", "", JOptionPane.ERROR_MESSAGE);
        }
    }
