package controlador.controladorSupervisor;

import java.awt.BorderLayout;
import java.awt.Color;
import java.awt.Dimension;
import java.awt.Font;
import java.util.List;

import javax.swing.BorderFactory;
import javax.swing.Box;
import javax.swing.JLabel;
import javax.swing.JPanel;

import modelo.crudPedidos.DaoPedido;
import modelo.crudPedidos.PedidoSupervisor;
import vista.vistaSupervisor.componentes.RoundedPanel;
import vista.vistaSupervisor.PedidosNoEntregados;

public class ControladorPediNoEn {

    private PedidosNoEntregados vpnoe = new PedidosNoEntregados();
    private JLabel lPNoEDesc;

    public ControladorPediNoEn(PedidosNoEntregados vpnoe) {
        this.vpnoe = vpnoe;
        crearPediNoEn();
    }

    public ControladorPediNoEn() {

    }

    private void crearPediNoEn() {

        DaoPedido rpdDao = new DaoPedido();

        // Obtener la lista y mostrar su contenido en consola
        List<PedidoSupervisor> lista = rpdDao.listarPNoE();
        if (lista == null || lista.isEmpty()) {
            JLabel vacio = new JLabel("No hay pedidos.");
            vacio.setOpaque(false);
            vacio.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 24));
            vpnoe.getPanelPedidoNoE().add(vacio);
        } else {
            for (PedidoSupervisor r : lista) {
                // crear un panel por petición con layout claro
                JPanel cardPedido = new RoundedPanel(12, Color.lightGray);
                cardPedido.setBackground(Color.white);
                cardPedido.setLayout(new BorderLayout(10, 10));
                cardPedido.setMaximumSize(new Dimension(900, 60));
                cardPedido.setPreferredSize(new Dimension(900, 60));
                if (cardPedido instanceof RoundedPanel) {
                    ((RoundedPanel) cardPedido).setShadowSize(3);
                    ((RoundedPanel) cardPedido).setShadowColor(new Color(0, 0, 0, 90));
                }

                lPNoEDesc = new JLabel(
                        "<html>El pedido " + r.getId() + " no fue entregado exitosamente </html>");
                lPNoEDesc.setPreferredSize(new Dimension(500, 300));
                lPNoEDesc.setBorder(BorderFactory.createEmptyBorder(10, 30, 10, 10));
                lPNoEDesc.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 16));

                cardPedido.add(lPNoEDesc, BorderLayout.WEST);

                vpnoe.getPanelPedidoNoE().add(cardPedido);
                vpnoe.getPanelPedidoNoE().add(Box.createRigidArea(new Dimension(0, 8)));
            }
        }

    }
}
