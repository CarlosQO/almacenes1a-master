...
public class ReportesOperativos extends JFrame {
    private JPanel panelP, panelRO;
    private Font customFont;
    private JLabel lReOp, lAño, lMes;
    private JTable tablaRO;
    private DefaultTableModel camposRO;
    ...
    public ReportesOperativos() {
        ...
        lReOp = new JLabel("Reportes Operativos");
        lReOp.setBounds(280, 0, 350, 50);
        panelRO.add(lReOp);
        ...
        btnBR = new JButton("Buscar Reportes");
        btnBR.setBounds(600, 90, 200, 40);
        panelRO.add(btnBR);

        add(panelRO);

        camposRO = new DefaultTableModel();
        camposRO.addColumn("ID");
        camposRO.addColumn("Concepto");
        camposRO.addColumn("Monto");
        camposRO.addColumn("Fecha");
        camposRO.addColumn("Medio de Pago");

        tablaRO = new JTable(camposRO);

        JTableHeader headerTabla = tablaRO.getTableHeader();
        ...
        tablaRO.setRowHeight(40);
        camposRO.addRow(new Object[5]);

        tablaRO.getColumnModel().getColumn(0).setMinWidth(0);
        tablaRO.getColumnModel().getColumn(0).setMaxWidth(0);
        tablaRO.getColumnModel().getColumn(0).setWidth(0);

        if (customFont != null) {
            tablaRO.setFont(customFont.deriveFont(Font.PLAIN, 16f));
            headerTabla.setFont(customFont.deriveFont(Font.PLAIN, 18f));
        } else {
            tablaRO.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 16));
            headerTabla.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 20));
        }

        // Renderer personalizado para las filas
        tablaRO.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override
        ...
        // ScrollPane
        jSPane = new JScrollPane(tablaRO);
